...
if [ -f "$CI_FILE" ]; then
  ...
  if [ "$G19_MATCHES" -eq 1 ]; then
    ...
    fail "G-19 ci.yml invokes run-gates.sh exactly once" "expected 1 exact invocation, found $G19_MATCHES"
  fi
else
  fail "G-19 ci.yml invokes run-gates.sh exactly once" "$CI_FILE missing"
fi
